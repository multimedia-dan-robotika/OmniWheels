...
"""
EXPERIMENT PENGENDALI MOTOR MENGGUNAKAN PWM ( <!> EXPERIMENTAL <!> )
NOTE : JIKA INGIN MENCOBA TESTING ROBOT OMNI, EKSEKUSI FILE "START.PY"!.

TASK : 
1. MEMBUAT CONTROL PWM
2. JOYSTICK PWM (JIKA MEMUNGKINKAN)
3. LOCK AND FOLLOWING TARGET
"""
...
from time import sleep # add sleep function library
...
def experimental():
    # PWM 
    pwm = GPIO.PWM(motorA1,1024)
    pwm = GPIO.PWM(motorA2,0)
    pwm = GPIO.PWM(motorB1,1024)
    pwm = GPIO.PWM(motorB2,0)
    pwm = GPIO.PWM(motorC1,0)
    pwm = GPIO.PWM(motorC2,0) 
